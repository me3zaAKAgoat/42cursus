...
/*   Updated: 2022/11/16 19:14:41 by echoukri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!read_str)
		return (free(static_str[fd]), free(read_str), NULL);
	while (look_for_newline(static_str[fd]) == -1)
	{
		bytes_read = read(fd, read_str, BUFFER_SIZE);
		if (bytes_read == 0 && gnl_strlen(static_str[fd]))
			break ;
		if (bytes_read == 0 && !gnl_strlen(static_str[fd]))
			return (free(read_str), free(static_str[fd])
				, static_str[fd] = NULL, NULL);
		if (bytes_read == -1)
			return (free(read_str), NULL);
		read_str[bytes_read] = 0;
		static_str[fd] = gnl_join(static_str[fd], read_str);
		if (!static_str[fd])
			return (free(static_str[fd]), free(read_str), NULL);
	}
