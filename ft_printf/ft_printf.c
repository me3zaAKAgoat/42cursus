...
/*   Updated: 2022/11/19 17:58:39 by echoukri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	conversion_decider(char format, va_list args, int *pto_count)
{
	if (format == 'c')
		...
		convert_str(va_arg(args, char *), pto_count);
	else if (format == 'p')
	{
		write(1, "0x", 2);
		*pto_count += 2;
		...
		ft_putslong(va_arg(args, unsigned int), "0123456789", pto_count);
	else if (format == 'x')
		ft_putslong(va_arg(args, unsigned int), "0123456789abcdef", pto_count);
	else if (format == 'X')
		ft_putslong(va_arg(args, unsigned int), "0123456789ABCDEF", pto_count);
	else if (format == '%')
	{
